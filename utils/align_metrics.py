...
class AlignmentMetrics:
    ...
    @staticmethod
    def cmd(x, y, contract="feat", eps=1e-8):
        # Correlation matrix distnance See https://ieeexplore.ieee.org/document/1543265
        if contract == "feat":
            cov1 = torch.matmul(x, x.transpose(0, 1))
            cov2 = torch.matmul(y, y.transpose(0, 1)) 
        else:
            # contract over batch
            cov1 = torch.matmul(x.transpose(0, 1), x)
            cov2 = torch.matmul(y.transpose(0, 1), y)
        return matrix_similarity(cov1, cov2, eps)
    
    @staticmethod
    def centered_cmd(x, y, contract="feat", eps=1e-8):
        centered_x = x - x.mean(dim=0)
        centered_y = y - y.mean(dim=0)
        return AlignmentMetrics.cmd(centered_x, centered_y, contract=contract, eps=eps)
...
def matrix_similarity(cov1, cov2, eps=1e-8):
    return torch.trace(torch.matmul(cov1, cov2)) / (cov1.norm() * cov2.norm() + eps)
